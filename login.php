...
if ($_POST) {
  ...
  $sentencia = $conexion->prepare("SELECT *, count(*) as n_usuarios FROM tbl_usuarios WHERE nombreusuario = :usuario AND password = :password");
  ...
  if ($registro['n_usuarios'] == 1) {
      ...
      $_SESSION['rol'] = $registro['rol']; // Almacenar el rol en la sesión

      // Redirigir según el rol
      if ($registro['rol'] == 0) { //administrador
          header("Location: index.php");

      } else if ($registro['rol'] == 1) { // Usuario
          header("Location: ./usuarios/empleados/");
      } else {
